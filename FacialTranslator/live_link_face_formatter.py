...
class LiveLinkFaceFormatter:
    ...
    def encode(self) -> bytes:
        """ Encodes the LiveLinkFaceFormatter object into a bytes object so it can be 
        send over a network. """              
    ...
    @staticmethod
    def decode(bytes_data: bytes) -> Tuple[bool, LiveLinkFaceFormatter]:
        """ Decodes the given bytes (send from an LiveLinkFaceFormatter App or from 
        this library) and creates a new LiveLinkFaceFormatter object.
        Returns True and the generated object if a face was found in the data, 
        False an a new empty LiveLinkFaceFormatter otherwise. 

        Parameters
        ----------
        bytes_data : bytes
            Bytes input to create the LiveLinkFaceFormatter object from.

        Returns
        -------
        bool
            True if the bytes data contained a face, False if not.        
        LiveLinkFaceFormatter
            The LiveLinkFaceFormatter object.
        ...
        if len(bytes_data) > name_end_pos + 16:
            ...
            live_link_face = LiveLinkFaceFormatter(name, uuid, fps)
            ...
            return False, LiveLinkFaceFormatter()
